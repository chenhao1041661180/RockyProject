...
public class LoadingMoreFooter extends LinearLayout {
    ...
    public final static int STATE_LOADDING_MORE = 3;
    ...
    public void setState(int state) {
        switch (state) {
            ...
            case STATE_NOMORE:
                ...
                this.setVisibility(View.VISIBLE);
                break;
            case STATE_LOADDING_MORE:
                if (mAnimationDrawable.isRunning()) {
                    mAnimationDrawable.stop();
                }
                mText.setText(getContext().getText(R.string.more_loading));
                mIvProgress.setVisibility(View.GONE);
                this.setVisibility(View.VISIBLE);
                break;
        }
...
//        if (getVisiableFooterHeight() > 0 || delta < 0) {
//            setVisiableHeight((int) delta + getVisiableFooterHeight());
//            if (mState <= STATE_RELEASE_TO_REFRESH) { // 未处于刷新状态，更新箭头
//                if (getVisiableFooterHeight() > mMeasuredHeight) {
...
//        }
    ...
    private int getVisiableFooterHeight() {
